...
def create_app(
    database="wikmt_db", echo=False, redirects=True, testing=False, csrf=True
):
    """
    Create a flask app
    ...
    def register():
        ...
        if request.method == "GET":
            ...
            return render_template(
                "register.html", form=register_form, login_form=login_form
            )
    ...
    def save_address():
        ...
        address, error = address_service.create_address(
            current_user.id, search_address, mapbox_id
        )
        if error:
            return jsonify({"success": False, "msg": error}), 400
        ...
        return jsonify(
            {"success": True, "msg": "Saved address", "address_id": address.id}
        )
    ...
    def delete_address():
        ...
        success, message, status_code = address_service.delete_address(
    ...
    def delete_search():
        ...
        success, message, status_code = search_service.delete_search(
            current_user.id, search_id
        )
